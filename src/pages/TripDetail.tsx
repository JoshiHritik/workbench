...
export default function TripDetail() {
  ...
  const hasGeneratedItinerary = Boolean(trip && loadCachedItinerary(trip.id))
  ...
  async function handleAddStop() {
    ...
    // Append directly from the insert response rather than relying solely on
    // a follow-up reload — keeps the new stop showing up immediately even if
    // the list re-fetch has to retry.
    setStops((prev) => [...prev, data as unknown as StopWithCity])
    setCitySearch('')
    ...
    <div className="min-h-svh bg-slate-50">
      ...
      <main className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
          ...
          {loadError && (
            <p role="alert" className="mt-2 rounded-md bg-red-50 px-3 py-2 text-sm text-red-600">
              Couldn't load stops: {loadError}
            </p>
          )}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="h-11 w-11 flex-shrink-0 overflow-hidden rounded-lg bg-slate-100">
                      {stop.cities.image_url ? (
                        <img src={stop.cities.image_url} alt={stop.cities.name} className="h-full w-full object-cover" />
                      ) : (
          ...
          {isOwner && (
            <div className="mt-4 border-t border-slate-100 pt-4">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="h-11 w-11 flex-shrink-0 overflow-hidden rounded-lg bg-slate-100">
                      {selectedCity.image_url && (
                        <img src={selectedCity.image_url} alt={selectedCity.name} className="h-full w-full object-cover" />
                      )}
                    </div>
                    <div>
                      <p className="text-sm font-medium text-slate-900">
                      ...
                      <p className="text-xs text-slate-500">{formatDateRange(stopStartDate, stopEndDate)}</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    ...
                    className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-slate-400 hover:bg-slate-200 hover:text-slate-700"
                  >
                    <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M6 6l12 12M6 18L18 6" />
                    </svg>
              ...
              <button
                ...
                disabled={addingStop || !selectedCity}
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {hasGeneratedItinerary
                  ? 'Your AI-suggested plan is ready to look at.'
            ...
            >
              {hasGeneratedItinerary ? 'View Itinerary' : 'Generate with AI'}
            </Link>
          </div>
        </div>
      </main>
